...
contract ArcProcure is ArcDefi {
    constructor(address _eurc) ArcDefi(_eurc) {}
    struct A { address seller; string item; uint256 minBid; uint256 highBid; address highBidder; uint256 endsAt; bool settled; }
    A[] public auctions;
    mapping(address => uint256[]) private mineMap;
    event Created(uint256 indexed id, address indexed seller, uint256 minBid);
    event Bid(uint256 indexed id, address indexed bidder, uint256 amount);
    event Settled(uint256 indexed id, address winner, uint256 amount);
    function create(string calldata item, uint256 minBid, uint256 durationDays) external returns (uint256 id) {
        require(durationDays > 0, "bad");
        id = auctions.length; auctions.push(A(msg.sender, item, minBid, 0, address(0), block.timestamp + durationDays * 1 days, false));
        mineMap[msg.sender].push(id); emit Created(id, msg.sender, minBid);
    }
    function bid(uint256 id) external payable {
        A storage a = auctions[id];
        require(block.timestamp < a.endsAt, "ended");
        require(msg.value >= a.minBid && msg.value > a.highBid, "low");
        address prev = a.highBidder; uint256 prevAmt = a.highBid;
        a.highBid = msg.value; a.highBidder = msg.sender;
        if (prev != address(0)) { (bool ok,) = payable(prev).call{value: prevAmt}(""); require(ok, "refund fail"); }
        emit Bid(id, msg.sender, msg.value);
    }
    function settle(uint256 id) external {
        A storage a = auctions[id];
        require(block.timestamp >= a.endsAt && !a.settled, "no");
        a.settled = true;
        if (a.highBidder != address(0)) { (bool ok,) = payable(a.seller).call{value: a.highBid}(""); require(ok, "fail"); }
        emit Settled(id, a.highBidder, a.highBid);
    }
    function get(uint256 id) external view returns (A memory) { return auctions[id]; }
